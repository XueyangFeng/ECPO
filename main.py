...
def simulate_user(crs_type, user_id, persona_path, config_path, format_path, user_model, crs_model, log_path, scratchpad_path, index_file, metadata_file, emb_model, conversation_rounds, crs_temperature, domain, query_num=5):
    is_correct = False
    item_id = 0
    ...
    #             scratchpad_data = {"Dialogue_id": f"User_id: {user_id}, Item_id: {item_id}", "scratchpad": scratchpad_reward(crs.get_traj(), rec_ratings, act_ratings, exp_ratings, rec_reasons, act_reasons, exp_reasons), "rec_rewards": rec_ratings, "act_rewards": act_ratings, "is_correct": is_correct}
    ...
    with open(scratchpad_path, mode="a", encoding="utf-8") as f:
            ...
            f.write(json.dumps(scratchpad_data) + "\n")               
    ## TODO, 这里只做了ActCRS的, 因为目前只计划微调ActCRS
...
def main():
    ...
    parser.add_argument("--query_num", type=int, default=5, help="Number of items to query.")
    ...
    with open(log_path, mode="a", encoding="utf-8") as f:
        if valid_users > 0:  # 如果有有效用户，进行汇总
            ...
            f.write(f"Total recall: {total_recall}/{valid_users}\n")
